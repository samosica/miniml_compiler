...
type 'a result = {
  before : (Vm.instr * (Vm.label * int), 'a) Map.t;
  after : (Vm.instr * (Vm.label * int), 'a) Map.t
}
...
let get_property rslt stmt side =
  match Map.get stmt
    ...
    Some p -> p
  | None -> err ("property not found: " ^ Vm.string_of_instr "" "\t" (fst stmt))
  ...
  let rec fixed_point wl entries exits =
    if Queue.is_empty wl then
      ...
      | GT ->
          ...
          let new_exit_prop = anlys.transfer new_entry_prop (fst stmt) in
          (* 差分の有無で場合分け *)
