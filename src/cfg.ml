...
type bblock = { mutable labels: Vm.label Set.t;
                stmts: (Vm.instr * (Vm.label * int)) array;
...
let string_of_cfg str_of_prop cfg =
  let fmt_stmt (s, _) = Vm.string_of_instr "  " "\t" s ^ "\n" in
...
let dot_of_cfg lbl str_of_prop cfg  =
  ...
  let fmt_stmt (s, _) = escape (Vm.string_of_instr "" "  " s ^ "\\l") in
  let fmt_prop s p = match str_of_prop with
      None -> ""
...
let coalesce_label lbl instrs =
  let lis = List.fold_left (fun stmts instr -> match stmts with
        [] -> (match instr with
            (Vm.Label l, _) -> [(Set.singleton l, None)]
          | _ -> [(Set.empty, Some instr)])
      | stmt :: stmts' ->
          (match (instr, stmt) with
             ((Vm.Label l, _), (lbls, None)) ->
               (Set.insert l lbls, None) :: stmts'
           | ((Vm.Label l, _), (_, Some _)) ->
  ...
  [(Set.singleton lbl, (Vm.BEGIN lbl, (lbl, -1)))] @
  (List.rev (List.map (function
         (lbls, Some instr) -> (lbls, instr)
       | (_, None) -> err "ill formed stmts") lis)) @ [(Set.empty, (Vm.END lbl, (lbl, -1)))]
...
let set_edges bbs used_lbls =
  ...
  List.iteri (fun i bb ->
      let (stmt, _) = bb.stmts.(Array.length bb.stmts - 1) in
      match stmt with
...
let vm_to_cfg lbl instrs =
  (* 命令にはあらかじめ番号を振っておく(1-indexed) *)
  let stmts = coalesce_label lbl (List.mapi (fun r i -> (i, (lbl, r + 1))) instrs) in
